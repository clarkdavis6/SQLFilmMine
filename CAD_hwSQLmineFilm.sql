...
SELECT first_name, last_name, SUM(amount) AS 'Aug2005Tot'
FROM staff INNER JOIN payment ON payment.staff_id = staff.staff_id
AND payment_date LIKE '2005-08%' GROUP BY first_name, last_name;

-- * 6c. List each film and the number of actors who are listed for that film. Use tables `film_actor` and `film`. Use inner join.
SELECT title, COUNT(actor_id) AS 'COUNT'
FROM film INNER JOIN film_id;

-- * 6d. How many copies of the film `Hunchback Impossible` exist in the inventory system?
-- USE DATABASE salila;
SELECT title, COUNT(title) as 'Copies' FROM film INNER JOIN inventory
